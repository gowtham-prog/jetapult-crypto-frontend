...
export default function CoinsTable({ coins, selectedId, onSelect, onToggleFavorite }) {
  const {
  ...
  const {
    ...
    error: removeError,
    execute: removeFavorite,
  } = useApi(deleteFavorite);
  const toggleFavourite = async (coin) => {
    try {
      if (coin.is_favorite) {
        await removeFavorite(coin.id);
        onToggleFavorite(coin.id, false); // inform parent
      } else {
        await addFavorite(coin.id);
        onToggleFavorite(coin.id, true); // inform parent
      }
    } catch (error) {
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-700/50">
            <tr>
              <th
                scope="col"
                className="px-4 sm:px-6 py-3 text-left font-medium text-gray-600 dark:text-gray-300"
              >
                Rank
              </th>
              <th
            ...
            {coins.map((coin) => {
              ...
              return (
                <tr
                  ...
                  className={`cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/60 transition-colors ${
                    ...
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          t.onerror = null;
                          t.src =
                            "https://placehold.co/32x32/111827/e5e7eb?text=%F0%9F%92%B0";
                        }}
                      />
                      <div>
